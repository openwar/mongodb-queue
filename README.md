...
const queue = new MongoDbQueue(db, 'queue');
```

To pass in options for the queue:

```js
const resizeQueue = new MongoDbQueue(db, 'resize-queue', {
  visibility: 30,
  delay: 15,
});
```

This example shows a queue with a message visibility of 30 seconds and a delay
to each message of 15 seconds.
...
### delay - Delay Messages on Queue in seconds

Default: `0`

When a message is added to a queue, it is immediately available for retrieval.
However, there are times when you might like to delay messages coming off a
queue. If you set delay to be `10`, then every message will only be available
for retrieval 10 seconds after being added.
...
```js
const id = await queue.add('Later', { delay: 120 });
// Message with payload 'Later' added.
// 'id' is returned, useful for logging.
// This message won't be available for consumption for 2 mins.
```
